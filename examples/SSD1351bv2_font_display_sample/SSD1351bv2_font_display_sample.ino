...
void setup() {
  Serial.begin(115200);
  oled.SSD1351bv2_Init(sclk, mosi, cs, DCpin, RSTpin); //SSD1351を初期化
  delay(300);
  oled.SSD1351bv2_BlackOut(); //黒画面出力
  ...
  for(j=0; j<7; j++){
    ...
    if(j < 4){
      //文字列を１列一気に表示させる
      oled.SSD1351bv2_8x16_DisplayOut_1col_LtoR(0, j*16, Red, Green, Blue, sj_length, font_buf);
    }else{
      for(i=0; i<16; i++){
        if(ci<2){
          Red = 31; Green = 0; Blue = 0;
        ...
        if(j < 6 ){
          //半角一文字ずつ表示させる
          oled.SSD1351bv2_8x16_DisplayOut(i*8, j*16, Red, Green, Blue, font_buf[i]);
        }else{
          if(i > 7) break;
          //２倍角フォント表示
          oled.SSD1351bv2_8x16_2x2_DisplayOut(i*16, 96, Red, Green, Blue, font_buf[i]);
        }
      }
    }
  }
  ...
  oled.SSD1351bv2_RAM_Hscrolle(0, 128, 0, 1); //(uint8_t StartY, uint8_t EndY, uint8_t Interval, uint8_t direction)
  sclTime = millis();
  contrastTime = millis();
}
...
void loop() {
  if(millis()-contrastTime > 5){
    ...
    oled.SSD1351bv2_RGBcontrast(R_ctr, G_ctr, B_ctr);//OLEDのコントラスト調整関数
    contrastTime = millis();
  }
  ...
  if(millis()-sclTime > 100){
    ...
    oled.SSD1351bv2_RAM_Vscrolle(scl++, 1); //(uint8_t StartY, uint8_t OffsetY)
    sclTime = millis();
  }
}
